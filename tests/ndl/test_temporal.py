...
def test_resource_lock_creation():
    with pytest.raises(temporal.NDLSyntaxError):
        req = temporal.ResourceLock(**{"ts": 0.0, "td": 1.0, "r": None})
    ...
    req_1 = temporal.ResourceLock(**{"ts": 0.0, "td": 10.0, "r": engaged(camera)})

    req_2 = temporal.ResourceLock(**{"ts": 0.0, "td": 10.0, "r": engaged(range)})

    req_3 = temporal.ResourceLock(**{"ts": 0.0, "td": 10.0, "r": engaged(bearing)})
...
def test_resource_level_creation():
    ...
    req_1 = temporal.ResourceLevel(**{"ts": 0.0, "td": 20.0, "r": direction(rov1), "n": L.constant(0, int_t)})
...
def test_action_creation():
    ...
    req_2 = temporal.ResourceLevel(**{"ts": 0.0, "td": 20.0, "r": direction(rov1), "n": L.constant(0, int_t)})
...
def test_instance_creation():
    ...
    req_4 = temporal.ResourceLevel(**{"ts": 0.0, "td": 20.0, "r": direction(rov1), "n": L.constant(0, int_t)})

    a1 = temporal.Action(
        name="localize_t0",
        parameters=[],
        precondition=position(rov1, p0),
        requirements=[TimedEffect(0.0, req_2), TimedEffect(0.0, req_3)],
        timed_effects=[TimedEffect(15.0, estimated_range(t0)), TimedEffect(20.0, estimated_bearing(t0))],
        untimed_effects=[],
    )
    ...
    a2 = temporal.Action(
        ...
        requirements=[TimedEffect(0.0, req_1), TimedEffect(0.0, req_2)],
        timed_effects=[TimedEffect(21.0, observed(t0))],
        untimed_effects=[],
    )
    ...
    inst = temporal.Instance(
        ...
        X=[
            position(rov1, p0),
            estimated_range(t0),
            estimated_bearing(t0),
            observed(t0),
            estimated_range(t1),
            estimated_bearing(t1),
            observed(t1),
            estimated_range(t2),
            estimated_bearing(t2),
            observed(t2),
        ],
        I=initial,
        A=[a1, a2],
