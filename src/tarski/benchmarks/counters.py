...
def generate_fstrips_counters_problem(ncounters=3, upper_bound=None):
    ...
    problem.action("increment", [c], precondition=value(c) < max_int(), effects=[value(c) << value(c) + 1])

    problem.action("decrement", [c], precondition=value(c) > 0, effects=[value(c) << value(c) - 1])
