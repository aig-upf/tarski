...
def test_bw_csp():
    ...
    assert (
        len(csp.constraints) == 2
        and str(csp.constraints[0]) == "StateVar(on(b1,b2)=True, reif=None)"
        and str(csp.constraints[1]) == "Table(<x> in ext(clear), negative=False, reif=None)"
    )
    ...
    assert (
        len(csp.constraints) == 2
        and str(csp.constraints[0]) == "StateVar(handempty()=True, reif=None)"
        and str(csp.constraints[1]) == "StateVar(handempty()=False, reif=None)"
    )
