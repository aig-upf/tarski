...
def test_parametrized_model_with_random_vars():
    ...
    the_task.add_constraint(
        forall(v, (ux(v) >= -1.0) & (ux(v) <= 1.0) & (uy(v) >= -1.0) & (uy(v) <= 1.0)), rddl.ConstraintType.ACTION
    )
...
def test_parametrized_model_with_random_vars_and_waypoints():
    lang = tarski.language(
        "lqg_nav_2d_multi_unit_waypoints", [Theory.EQUALITY, Theory.ARITHMETIC, Theory.SPECIAL, Theory.RANDOM]
    )
    ...
    the_task.add_cpfs(
        wv(wpt),
        ite(sqrt(dist_vec_norm) <= wr(), lang.constant(1.0, lang.Real), max(lang.constant(0.0, lang.Real), wv(wpt))),
    )
    the_task.add_cpfs(vx(v), vx(v) + dt() * ux(v) + normal(mu_w(), sigma_w()))
    the_task.add_cpfs(vy(v), vy(v) + dt() * uy(v) + normal(mu_w(), sigma_w()))
    the_task.add_cpfs(x(v), x(v) + dt() * vx(v))
...
def test_parametrized_model_with_random_vars_and_waypoints_boolean():
    lang = tarski.language(
        "lqg_nav_2d_multi_unit_bool_waypoints", [Theory.EQUALITY, Theory.ARITHMETIC, Theory.SPECIAL, Theory.RANDOM]
    )
