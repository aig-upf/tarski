...
class Predicate(object):
    ...
    def __call__(self, *args):
        if len(args) != self.arity:
            raise LanguageError('Error binding predicate to terms to construct relational formula: '
                                'arity of {} is {}, {} arguments given'.format(self.symbol, self.arity, len(args)))
        for k, arg in enumerate(args):
            ...
            if arg.type.name != self._type[k].name:
                raise LanguageError(
                    'Error binding predicate: {}-th argument type mismatch, type is {}, was expected to be {}'.format(
                        k + 1, arg.type.name, self._type[k].name))
        ...
        for k, arg in enumerate(args):
            ...
            if arg.type.name != self._type[k].name:
                raise LanguageError('Error setting predicate extension: {}-th argument type mismatch, type is {}, '
                                    'was expected to be {}'.format(k + 1, arg.type.name, self._type[k].name))
