...
class Sort:
    ...
    def to_constant(self, x):
        ...
        from . import Constant, Variable  # Avoiding circular references
    ...
    def dump(self):
        return dict(name=self.name, domain=list(self._domain))  # Copy the list
...
class Interval(Sort):
    ...
    def domain(self):
        ...
        from . import Constant  # Avoiding circular references
...
def build_the_reals(lang):
    reals = Interval("Real", lang, float_encode_fn, -3.40282e38, 3.40282e38, builtin=True)
    reals.builtin = True
    return reals
