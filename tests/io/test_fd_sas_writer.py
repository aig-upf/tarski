...
def test_gripper_instance():
    ...
    the_balls = [L.constant(f"ball{i}", ball_t) for i in range(4)]
    ...
    with tempfile.TemporaryFile(mode="w+") as outstream:
        Writer(theory=L2, state_variables=X, domains=D, actions=Act, initial=initial_state, goal=goal_state).dump(
            outstream
        )
        outstream.seek(0)
