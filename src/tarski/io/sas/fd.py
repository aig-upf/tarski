...
from tarski.io.sas.templates import (
    axiom_defaults_section_tmpl,
    effect_elem_tmpl,
    goal_state_section_tmpl,
    goal_value_list_elem_tmpl,
    initial_state_section_tmpl,
    initial_value_list_elem_tmpl,
    operator_tmpl,
    precondition_elem_tmpl,
    sas_instance_tmpl,
    variable_section_elem_tmpl,
)
...
class Writer:
    ...
    def get_sas_version(self) -> str:
        return f"{FAST_DOWNWARD_SAS_VERSION}"

    def get_metric_type(self) -> str:
        return f"{self.unit_costs and 0 or 1}"

    def get_number_variables(self) -> str:
        return f"{len(self.state_variables)}"
    ...
    def make_variable_section(self) -> str:
        ...
        for var in self.state_variables:
            elements += [
                variable_section_elem_tmpl.substitute(
                    name=str(var.expr),
    ...
    def make_axiom_defaults_section(self) -> str:
        ...
        return axiom_defaults_section_tmpl.substitute(
            num_axiom_vars=0, axiom_defaults_list=self.make_axioms_defaults_list()
        )
    ...
    def make_precondition_list(self, a):
        ...
        for x, v, _ in a.transitions:
            elems += [
                precondition_elem_tmpl.substitute(
    ...
    def make_initial_value_list(self) -> str:
        ...
        for x, v in self.initial:
            values += [
                initial_value_list_elem_tmpl.substitute(
                    variable_index=self.state_variables.get_index(symref(x)),
                    value_index=self.domains[symref(x)].get_index(symref(v)),
                )
            ]
    ...
    def make_initial_state_section(self) -> str:
        return initial_state_section_tmpl.substitute(
            num_variables=len(self.state_variables), initial_value_list=self.make_initial_value_list()
        )
    ...
    def make_goal_value_list(self) -> str:
        ...
        for x, v in self.goal:
            values += [
                goal_value_list_elem_tmpl.substitute(
                    variable_index=self.state_variables.get_index(symref(x)),
                    value_index=self.domains[symref(x)].get_index(symref(v)),
                )
            ]
    ...
    def make_goal_state_section(self) -> str:
        return goal_state_section_tmpl.substitute(goal_value_list=self.make_goal_value_list())
