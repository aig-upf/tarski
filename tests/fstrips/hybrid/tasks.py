...
def create_particles_world():
    ...
    constraint_1 = task.differential_constraint('test1', [p_var], top, x(p_var), f(p_var)*2.0)
    cond = (x(p_var) > 0.0) & (x(p_var) < 0.5)
    constraint_2 = task.differential_constraint('test2', [p_var], cond, y(p_var), f(p_var)*0.5)
...
def create_billiards_world():
    ...
    constraint = task.reaction('principle_of_superposition', \
