...
def create_parcprinter_language():
    upp = tsk.language()
...
def create_small_task():
    ...
    upp = create_parcprinter_language()
    ...
    action = fs.Action( upp, name='Finisher2-Stack-Letter',\
                        parameters = [sheet, prevsheet],\
                        ...
                        effects = [ ltl.X( land(\
                                            neg(upp.Available(upp.finisher2_rsrc)),\
                                            neg(upp.Location(sheet, upp.finisher2_entry_finisher1_exit)),\
                                            ltl.X( land(\
                                            upp.Location(sheet, upp.some_finisher_tray),\
                                            upp.Stackedin(sheet, upp.finisher2_tray),\
                                            upp.Available(upp.finisher2_rsrc)))))])
    P = fs.Problem("fun", upp, init=M, goal=top, constraints = [], actions=[action])
    return P
