...
def test_action_grounding_bw():
    ...
    b1, b2, b3, clear, on, ontable, handempty, holding = problem.language.get(
    ...
    assert isinstance(ground, PlainOperator) and str(ground.precondition) == "(on(b1,b2) and clear(b1) and handempty())"
...
def test_complex_action_grounding():
    ...
    action = problem.action("fake", [x], precondition=handempty(), effects=[ue])
