...
def generate_strips_bw_language(nblocks=4):
    ...
    _ = [lang.predicate(p, object_t) for p in ["ontable", "clear", "holding"]]
...
def generate_strips_blocksworld_problem(nblocks=4, use_inequalities=True):
    ...
    problem.action(
        ...
        effects=[fs.DelEffect(ontable(x)), fs.DelEffect(clear(x)), fs.DelEffect(handempty()), fs.AddEffect(holding(x))],
    )
    ...
    problem.action(
        ...
        effects=[fs.AddEffect(ontable(x)), fs.AddEffect(clear(x)), fs.AddEffect(handempty()), fs.DelEffect(holding(x))],
    )
...
def generate_fstrips_blocksworld_problem(nblocks=4, init="random", goal="random"):
    ...
    problem.action(
        ...
        effects=[loc(x) << y, AddEffect(clear(loc(x))), DelEffect(clear(y), condition=(y != table))],
    )
    return problem
